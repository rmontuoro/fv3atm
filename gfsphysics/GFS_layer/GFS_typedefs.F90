...
!--- tracer handling
    ...
    integer              :: ntche           !< tracer index for last chemical tracer, including diagnostic
...
!-------------------------
  subroutine coupling_create (Coupling, IM, Model)
    ...
    if (Model%cplflx .or. Model%cplchm .or. Model%cplaqm &
        .or. Model%do_sppt .or. Model%ca_global) then
...
!--- tracer handling
    ...
    if (Model%ntchs > 0) then
      if (cplchm) then
        ...
        Model%ntche        = get_tracer_index(Model%tracer_names, 'pm25co',   Model%me, Model%master, Model%debug)
